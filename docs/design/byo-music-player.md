...
- Storage backend is user-configured in Settings, not hardcoded: S3 (top
  priority), Google Drive, Dropbox, OneDrive — extensible to more later.
...
- No WebDAV provider — dropped from scope; the four adapters being built
  (S3, Google Drive, Dropbox, OneDrive) already prove out the `CloudProvider`
  protocol's extensibility.
...
top priority) and Google Drive (`GoogleSignIn-iOS`), Dropbox
(`SwiftyDropbox`), and OneDrive (`MSAL` + Graph API) adapters. First-party
Keychain wrapper over `Security` for credentials. `GRDB.swift` local index
...
- Dropbox's SwiftyDropbox and Microsoft's MSAL both expect a registered
  ...
  scheme for the OAuth redirect — one-time setup docs needed in Settings
  help text, same as the S3/Drive credential guidance.
